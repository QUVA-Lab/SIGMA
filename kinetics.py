...
class VideoClsDataset(Dataset):
    ...
    def __init__(self, anno_path, data_path, mode='train', clip_len=8,
        ...
        if 'UCF' in args.data_set:
                ...
                #print("samples",self.dataset_samples[0:10])
                #print("labels",self.label_array[0:10])

                from utils_datasets import get_filenames_and_labels_ucf
        ...
        if 'DIV' in args.data_set:
                ...
                from utils_datasets import get_filenames_and_labels_diving

                self.dataset_samples,self.label_array  = get_filenames_and_labels_diving(self.anno_path,mode)

        if 'GYM' in args.data_set:

                from utils_datasets import get_filenames_and_labels_gym
        ...
        if 'FXS1' in args.data_set:
                ...
                from utils_datasets import get_filenames_and_labels_gym_fx_s1

                self.dataset_samples,self.label_array  = get_filenames_and_labels_gym_fx_s1(self.anno_path,mode)
        if 'UBS1' in args.data_set:

                from utils_datasets import get_filenames_and_labels_gym_ub_s1
...
class VideoMAE(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # its faster when try except only here 
        except Exception:
            print(" failed loading features for: ",path)
