...
def final_test(data_loader, model, device, file):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            output = model(videos)
            loss = criterion(output, target)
        #print("output size, ids, chunk_nb,split_nb", output.size(),ids,chunk_nb,split_nb)
...
def merge_mean_per_class(eval_path, num_tasks,nb_classes):
    ...
    for c in classes:
         ...
         #print(top1)
         final_top1 ,final_top5 = np.mean(top1), np.mean(top5)
         #print(final_top1,final_top5)
         classwise_top1[c] = final_top1
         classwise_top5[c] = final_top5

    print(classwise_top1)
    classwise_top1 = np.mean(classwise_top1)*100
    classwise_top5 = np.mean(classwise_top5)*100
    #print(classwise_top1,classwise_top5)
